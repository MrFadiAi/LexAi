// Background service worker (background.js) - v0.4.1 - Tone/Lang for Reply
...
const STORAGE_KEY_TONE = 'aiCorrectorLastTone'; // Added for loading default tone
const DEFAULT_MODEL = 'gemini-2.0-flash'; 
const DEFAULT_TONE = 'neutral'; // Added default tone
const GEMINI_API_BASE_URL = "https://generativelanguage.googleapis.com/v1beta/models/";

// --- Variables (Loaded from Storage) ---
let GEMINI_API_KEY = ""; 
let AI_MODEL = DEFAULT_MODEL; 
let LAST_TONE = DEFAULT_TONE; // Variable to hold last used tone

// --- Load Initial Settings from Storage ---
async function loadSettings() { 
    console.log("BG Script: Loading settings from storage..."); 
    try { 
        // Load all settings at once
        const result = await chrome.storage.sync.get([STORAGE_KEY_API, STORAGE_KEY_MODEL, STORAGE_KEY_TONE]); 
        
        GEMINI_API_KEY = result[STORAGE_KEY_API] || ""; 
        AI_MODEL = result[STORAGE_KEY_MODEL] || DEFAULT_MODEL; 
        LAST_TONE = result[STORAGE_KEY_TONE] || DEFAULT_TONE; // Load last tone

        console.log("BG Script: Loaded API Key:", GEMINI_API_KEY ? '******' : '(Not Set)');
        console.log("BG Script: Loaded Model:", AI_MODEL);
        console.log("BG Script: Loaded Last Tone:", LAST_TONE); // Log loaded tone

    } catch (error) { 
        console.error("BG Script: Error loading settings from storage:", error); 
        GEMINI_API_KEY = ""; 
        AI_MODEL = DEFAULT_MODEL;
        LAST_TONE = DEFAULT_TONE; // Reset tone on error too
    } 
}
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  if (port.name === 'ai-corrector') {
    port.onMessage.addListener((msg) => {
      ...
      // --- Handler for Full Suggestion/Translation ---
      if (msg.action === "checkText") {
        const { text: textToCheck, tone: requestedTone = LAST_TONE, language: requestedLanguage = 'English' } = msg; // Use LAST_TONE as default
      ...
      // --- Handler for Generate Reply ---
      else if (msg.action === "generateReply") {
          // Use tone/language from message if provided, otherwise use defaults/last used
          ...
          (async () => {
              try {
                  ...
                  // Pass tone and language to getAIReply
...
async function getAISuggestion(text, tone = LAST_TONE, language = 'English') { // Use LAST_TONE as default
  ...
  let toneInstruction = `Apply a '${tone}' tone to the text.`; if (tone.toLowerCase() === 'funny') { toneInstruction = `Rewrite the text with a funny, slightly swag, internet/twitter humor style, like it's a witty online comment. Keep it concise and natural-sounding.`; } 
...
async function getAIReply(originalText, tone = LAST_TONE, language = 'English') { // Add tone and language params
    ...
    if (tone.toLowerCase() === 'funny') {
        toneInstruction = `The reply should have a funny, slightly swag, internet/twitter humor style, like it's a witty online comment.`;
    }
